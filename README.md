...
## Requirements
GAE-LGA is tested in the conda environment. It is recommended that users test the program in this way. Of course, they can also execute the program in a familiar environment. Note: Before using GAE-LGA, users first need to prepare the following softwares in the execution environment：
  * Python 3.6.2
  * PyTrorch 1.9.1
  * NumPy 1.19.2
  * scikit-learn 0.24.1
...
### 2: This directory is used to store the experimental results of the LncTarD dataset
  ...
  * test_new_assoc.txt
  > New lncRNA-PCG associations identified on the test set.
### 3: This directory is used to store the experimental results of the LncRNA2Target dataset
  * train_rec_pre.txt 
  > Changes in recall and precision on the training set.
  * train_perform.txt 
  > Model performance on the training set
