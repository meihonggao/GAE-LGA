...
def train(model, model2, dataset, args):
    ...
    df.to_csv(args.result_path+'/train_perform_8.txt',sep=':',header=0)    
    ...
    np.savetxt(args.result_path+'/train_fpr_tpr_8.txt', fpr_tpr,delimiter=',')
    np.savetxt(args.result_path+'/train_rec_pre_8.txt', rec_pre,delimiter=',')
...
def test(model,model2,dataset,args):
    ...
    print('Recall',Rec)
    df=DataFrame([auroc,aupr,acc,F1,Pre,Rec],index=['AUC','AUPR','ACC','F1-score','Precision','Recall'],columns=['peformance']) 
    df.to_csv(args.result_path+'/test_perform_8.txt',sep=':',header=0)    
    fpr_tpr=np.vstack((fpr,tpr))
    rec_pre=np.vstack((recall,precision))
    np.savetxt(args.result_path+'/test_fpr_tpr_8.txt', fpr_tpr,delimiter=',')
    np.savetxt(args.result_path+'/test_rec_pre_8.txt', rec_pre,delimiter=',')
...
def main():
    ...
    args.dataset_path='../Datasets/Dataset1'    #Dataset1,Dataset2,Dataset3
    args.result_path='../Results/1'    #1,2,3
    args.fl=263 #263,238,308
    args.fg=498 #498,716,256
    args.epoch=18336
