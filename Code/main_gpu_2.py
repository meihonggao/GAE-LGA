...
def train(model, model2, dataset, args):
    ...
    train_data['x_data_matrix'],train_data['x_edge_index']=\
        torch.cat((dataset['ll_f']['data_matrix'],dataset['ll_m']['data_matrix']),1).cuda(), dataset['ll_f_train']['edge_index'].cuda()
    train_data['y_data_matrix'],train_data['y_edge_index'] =\
        torch.cat((dataset['gg_f']['data_matrix'],dataset['gg_m']['data_matrix']),1).cuda(), dataset['gg_f_train']['edge_index'].cuda()
    ...
    df.to_csv(args.result_path+'/train_perform_16.txt',sep=':',header=0)    
    ...
    np.savetxt(args.result_path+'/train_fpr_tpr_16.txt', fpr_tpr,delimiter=',')
    np.savetxt(args.result_path+'/train_rec_pre_16.txt', rec_pre,delimiter=',')
...
def test(model,model2,dataset,args):
    ...
    test_data['x_data_matrix'],test_data['x_edge_index']=\
        torch.cat((dataset['ll_f']['data_matrix'],dataset['ll_m']['data_matrix']),1).cuda(), dataset['ll_f_test']['edge_index'].cuda()
    test_data['y_data_matrix'],test_data['y_edge_index'] =\
        torch.cat((dataset['gg_f']['data_matrix'],dataset['gg_m']['data_matrix']),1).cuda(), dataset['gg_f_test']['edge_index'].cuda()
    model.eval() 
    ...
    print('Recall',Rec)
    df=DataFrame([auroc,aupr,acc,F1,Pre,Rec],index=['AUC','AUPR','ACC','F1-score','Precision','Recall'],columns=['peformance']) 
    df.to_csv(args.result_path+'/test_perform_16.txt',sep=':',header=0)    
    fpr_tpr=np.vstack((fpr,tpr))    
    rec_pre=np.vstack((recall,precision))
    np.savetxt(args.result_path+'/test_fpr_tpr_16.txt', fpr_tpr,delimiter=',')
    np.savetxt(args.result_path+'/test_rec_pre_16.txt', rec_pre,delimiter=',')
...
def main():
    ...
    args.dataset_path='../Datasets/Dataset2'    #Dataset1,Dataset2,Dataset3
    args.result_path='../Results/2'    #1,2,3
    args.fl=238 #263,238,308
    args.fg=716 #498,716,256
    args.epoch=1314
