...
def train(model, model2, dataset, args):
    ...
    print('Recall',Rec)
    print('MCC',Mcc)
    df=DataFrame([auroc,aupr,acc,F1,Pre,Rec,Mcc],index=['AUC','AUPR','ACC','F1-score','Precision','Recall','MCC'],columns=['peformance']) 
...
def test(model,model2,dataset,args):
    ...
    np.savetxt(args.result_path+'/test_new_assoc.txt', new_assoc.T, delimiter=',',fmt='%1.2f')
    fpr, tpr, thresholds = roc_curve(y_true, y_score)
    precision, recall, _ = precision_recall_curve(y_true, y_score)
    auroc=auc(fpr, tpr)
    aupr=auc(recall, precision)
...
def main():
    ...
    args.fl=238 #308,238,263
    args.fg=716 #256,716,498
    args.epoch=1314
